...
namespace GameHex
{
	...
	const AbstractHex::Cell& HexVector::operator()(int i, int j, int key) const
	{
		if(key == 0)	
			return hexCells[i][j];

		else if(key == 1)
			return hexCellsX[i][j];

		else
			return hexCellsO[i][j];		
	}




	void HexVector::update_hexCells_X( )
	{
	int i,j;
	...
	for (i = 0; i < getSize(); ++i)
	{
		for (j = 0; j < getSize(); ++j)
		{
			...
			else if((j == getSize()-1) && hexCells[i][j].getChar( ) == 'x')
			{
...
void HexVector::update_hexCells_O( )
{
	int i,j;
	...
	for (i = 0; i < getSize(); ++i)
	{
		for (j = 0; j < getSize(); ++j)
		{
			...
			else if((i == getSize()-1) && hexCells[i][j].getChar( ) == 'o')
			{
...
int HexVector::isEnd_X(int rows, int column)
{
...
return 0;
	...
	int HexVector::isEnd_O(int rows,int column){
	...
	void HexVector::make_winner_path_upperX( )	//bura 
	{
		int i,j;
		for (i = 0; i < getSize(); ++i)
		{
...
void HexVector::make_winner_path_upperO( )
	{
		int i,j;
		for (i = 0; i < getSize(); ++i)
		{
	...
	int HexVector::readFromFile(string &file_name)		//Load file
	{
		ifstream loadFile;
...
int HexVector::play(int xxx)	// int xxx just for make overload.
{
		int random_row;
		int random_col;
		...
		// I observed that trying to capture middle of the game is important so that I am trying to achive that.
			...
			if (hexCells[getSize()/2][getSize()/2].getChar( ) == '*')
			{
				hexCells[getSize()/2][getSize()/2].setCharO( );
				return 1;				
			}

			if(hexCells[(getSize()/2)+1][getSize()/2].getChar( ) == '*'){
				hexCells[(getSize()/2)+1][getSize()/2].setCharO( );
				return 1;			
			}
	...
	if (getCol()+1 < getSize())
	{
		...
		{		
			hexCells[getRow()][getCol()+1].setCharO( );
			return 1;
		}

		if (check_location_AI(getRow()-1,getCol()+1) == 1)
		{
			hexCells[getRow()-1][getCol()+1].setCharO( );
			return 1;
		}
		...
		{
			hexCells[getRow()-1][getCol()].setCharO( );			
			return 1;
		}
...
while(getSize()*getSize() > getCount()){
	...
	while(getSwitchplayer() == 1 && getCount() < 1000)
		...
		if(getStr1() !="QUIT" || getStr2() !="GAME"){	
			...
			//else if(getStr1() == "undo" && getStr2() == "move")	// dewam
			//{
				//flag = 1;
				//--*this;
				//cout <<*this;
			...
			else if(split_string( ) == 1)
			{
				if (check_letter( ) == 1 && check_number( ) == 1)
				{
					...
					if(check_location( ) == 1)
					{
						hexCells[getRow()][getCol()].setCharX( );
						//displayBoard( );
						print();
						...
						if(control_start_X() == 1 && control_finish_X() == 1)
						{
							...
							{
								if (hexCellsX[getFindstartX()][0].getInfo( ) == Cellinfo::start_X)	//Start point will be on First column and I must check all start points.
								{
			...
			else if(split_string( ) == 1)
			{
				if (check_letter( ) == 1 && check_number( ) == 1)
				{
					...
					if(check_location( ) == 1)
					{
						...
						hexCells[getRow()][getCol()].setCharO( );
						...
						if(control_start_O() == 1 && control_finish_O() == 1)
						{
							...
							{
								if (hexCellsO[0][getFindstartO()].getInfo( ) == Cellinfo::start_O)	//Start point will be on First column and I must check all start points.
								{

...
void new_board (char board[MAX_SIZE][MAX_SIZE],char letters[],int size, char opponent_choice, int marked_X[MAX_SIZE][MAX_SIZE], int marked_O[MAX_SIZE][MAX_SIZE])
{
	...
	{	
		...
		while(switch_player == 1)
		{
			...
			cin >> str1 >> str2;
			if(split_string(str1,str2, &letter_user,&number)==2){

				if (str1 == "LOAD")
				{
					if(load_game(board,marked_X,marked_O,&size,&switch_player,&opponent_choice,str2)==1){
					cout <<"\n\n ***Game is loaded!***" << endl;
					...
					if(save_game(board,marked_X,marked_O,size,switch_player,opponent_choice,str2)==1)
					cout <<"\n ***Game is saved!***" << endl;
...
int main(){
	...
	while(control_size>0)
	{
		...
		{
			control_size=1;
			cerr << "!Number must between six and twelve!\n";
